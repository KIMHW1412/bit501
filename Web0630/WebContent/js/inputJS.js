...
function check() {
	...
	if (code == null || code == "" || juso1 == null || juso1 == ""
			|| juso2 == null || juso2 == "") {
		alert("주소를 입력하세요");
		iform.code.focus();
		return;
	...
	// 위에 해당 되는거 없으면 null 체크 true
	flag = true;

	if (flagS == false) { // 사번중복체크 안됐을 때
		alert("사번 중복체크 하세요.");
		return;
	}

	if (flagP == false) { // 비번확인 틀렸을때
		alert("비밀번호가 일치하지 않습니다.");
		iform.pwd2.focus();
		return;
	}

	if (flagN == false) { // 전화번호 올바르지 않을 때
		alert("전화번호를 올바르게 입력하세요.");
		return;
	}

	// 위에 다 통과&체크 완료 후 submit
	if (flag == true && flagS == true && flagP == true && flagE == true
			&& flagN == true) {
...
function checkSb() { // 사번 입력체크하고 중복확인 팝업창 열어주기
	...
	label.innerHTML = "";
	open('openID.jsp', 'openID', 'width=400, height=120, left=200, top=200');
	flagS_t();
} // checkSb end
...
function checkPwd() { // 비밀번호 확인 체크
	...
	if (iform.pwd.value == iform.pwd2.value) {
		...
		flagP = true; // 비밀번호일치 플래그 true로 해줌
	} else {
...
function DaumPostcode() {
	new daum.Postcode({
		oncomplete : function(data) {
			...
			document.getElementById('code').value = data.zonecode; // 5자리 새우편번호
			// 사용
...
function num() {
	...
	if (test1 == false || test2 == false || test3 == false) {
		label.innerHTML = "&nbsp;&nbsp;숫자 3~4자리 입력하세요.";
	} else {
		label.innerHTML = "";
		flagN = true;
	}
...
function handleFileSelect() {
	...
	reader.onload = (function(theFile) {
		return function(e) {
			// 이미지뷰
			var img_view = [ '<img src="', e.target.result, '"name="',
